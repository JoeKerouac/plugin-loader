...
import com.github.joekerouac.plugin.loader.data.RandomAccessDataFile;

/**
 * Extended variant of {@link java.util.jar.JarFile} that behaves in the same way but offers the following additional
 * functionality.
 * <ul>
 * <li>A nested {@link JarFile} can be {@link #getNestedJarFile(ZipEntry) obtained} based on any directory entry.</li>
 * <li>A nested {@link JarFile} can be {@link #getNestedJarFile(ZipEntry) obtained} for embedded JAR files (as long as
 * their entry is not compressed).</li>
 * </ul>
 *
 * @author JoeKerouac
...
public class JarFile extends AbstractJarFile implements Iterable<java.util.jar.JarEntry> {
    ...
    private static final String HANDLERS_PACKAGE = "com.github.joekerouac.plugin.loader";
    ...
    /**
     * Private constructor used to create a new {@link JarFile} either directly or from a nested entry.
     * 
     * @param rootFile
     *            the root jar file
     * @param pathFromRoot
    ...
    /**
     * Register a {@literal 'java.protocol.handler.pkgs'} property so that a {@link URLStreamHandler} will be located to
     * deal with jar URLs.
     */
    ...
    /**
     * Reset any cached handlers just in case a jar protocol has already been used. We reset the handler by trying to
     * set a null {@link URLStreamHandlerFactory} which should have no effect other than clearing the handlers cache.
     */
